...
export default function EnquireNow() {
  ...
  const dateFormatting = async () => {
    ...
    await axios.get(`${apiUrl}&timeMin=${formattedMinDate}`).then(({
      data
    }) => {
      const {
        items
  ...
  const disabledDates = date => {
    ...
    if (days[day] === "Saturday") {
      ...
      const indicesArr = dateLists.dateObjectList?.dates.map((date, index) => {
      ...
      for (let i = 0; i < indicesArr?.length; i++) {
        ...
        selectedDateTimes[(dateLists.dateObjectList.dateTimes.start[indicesArr[i]] / 10000).toFixed()].push(minutes);
      }
      const allTimesBooked = Object.values(selectedDateTimes).every(arr => arr.length === 2);
      if (allTimesBooked) {
        return true;
      } else {
        return false;
  ...
  const disabledTimes = date => {
    ...
    let hour = date["$H"];
    const formattedDate = `${date["$y"]}${date["$M"] + 1 < 10 ? "0" + (date["$M"] + 1) : date["$M"] + 1}${date["$D"] < 10 ? "0" + date["$D"] : date["$D"]}`;
    if (days[day] === "Sunday" && !dateLists.dateObjectList?.dates.includes(Number(formattedDate))) {
      if (!(hour >= 9 && hour <= 16)) {
        return true;
      }
    } else if (days[day] === "Sunday" && dateLists.dateObjectList?.dates.includes(Number(formattedDate))) {
      let minute = date["$m"];
      const indicesArr = dateLists.dateObjectList?.dates.map((date, index) => {
        if (date === Number(formattedDate)) {
          return index;
  ...
  return /*#__PURE__*/React.createElement("section", {
    ...
    variant: "body"
  }, "* Note: on Sunday we do in person quotations")), /*#__PURE__*/React.createElement(LocalizationProvider, {
